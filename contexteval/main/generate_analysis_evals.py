...
_CONTEXT_NAME = flags.DEFINE_string(
    "context_name", "", "Name of contextual attribute."
)
_EVAL_MODEL = flags.DEFINE_string(
    "eval_model", "gemini-1.5-pro", "Model to use for evaluation."
)
...
def main(unused_argv) -> None:
  ...
  for ex in tqdm.tqdm(candidate_outputs):
    if "context" in ex:
      ...
      outputs.append({"query": ex["query"], "response": ex["response"], "context": ex["context"], "eval_judgement": eval_judgement, "is_default_response": False})
      ...
      output_format = "**output: "  + str({k: {"relevance": "_"} for k in answer_list}).replace("'_'", "_") + "**"
      cur_prompt = (eval_prompt + ".")[:-1]
      ...
      cur_prompt = cur_prompt.replace("[RESPONSE]", ex["response"])
      eval_judgement = get_eval_judgement(cur_prompt, model)
      outputs.append({"query": ex["query"], "response": ex["response"], "context": context, "eval_judgement": eval_judgement, "is_default_response": True})
      jsonl_utils.write(OUTPUT_PATH, [outputs[-1]], append=True, verbose=False)
