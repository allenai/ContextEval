...
_CONTEXT_NAME = flags.DEFINE_string("context_name", None, "Name of the contextual attribute.")
_MODEL_NAME = flags.DEFINE_string("model_name", "gpt-4", "Model name.")
...
def get_qa(input_str):
    ...
    question = question_match.group(1) if question_match is not None else "Question not found"
    list_match = re.search(r"A: (\[.*\])", input_str)
    answer_list_str = list_match.group(1) if list_match is not None else "[]"
...
def main(unused_argv) -> None:
    ...
    sampled_examples = random.sample(
        examples_by_context[_CONTEXT_NAME.value],
        min(MAX_QUERIES, len(examples_by_context[_CONTEXT_NAME.value])),
    )
    outputs = []
    for ex in tqdm.tqdm(sampled_examples):
        if _DEFAULT.value:
            ...
            for answer in answer_list:
                ...
                outputs.append(
                    {"query": ex["query"], "context": context, "response": context_response}
                )
                jsonl_utils.write(OUTPUT_PATH, [outputs[-1]], append=True, verbose=False)
