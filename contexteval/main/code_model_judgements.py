...
def get_eval_justifications(eval_models, filename_template):
  ...
  for i, ex in enumerate(eval_data["Qwen_Qwen2-72B-Instruct"]):
    qualified_models = [model for model in eval_models if "Justification" in eval_data[model][i]["eval_judgement"]]
    ...
    if "context" in ex:
      ...
      context = ""
    eval_justifications.append({"judgement": eval_data[rand_model][i]["eval_judgement"],
                                "query": ex["query"],
                                "context": context,
                                "setting": filename_template.split("/")[-1].split("_EVAL_MODEL")[0].replace("sampled_evals_", ""),
