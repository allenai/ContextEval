  ...
  for model in eval_models:
    ...
    for ex in eval_data:
      ...
      if dict_out is None or "judgement" not in dict_out:
        ...
        elif "**Response 2**" in ex["eval_judgement"] or "Response 2" in ex["eval_judgement"]:
...
def get_agreement(eval_judgements, eval_models, selected_idxs=None):
  ...
  for i, idx in enumerate(selected_idxs):
    model_judgements = [eval_judgements[model][idx] if eval_judgements[model][idx] != "Invalid output" else TIE_WORD for model in eval_models]
    ...
    if most_common_judgement != TIE_WORD:
      non_tie_instance_wise_agreements.append(most_common_judgement_count / len(model_judgements))
    if most_common_judgement_count / len(model_judgements) > 0.5:
      majority_judgements.append(most_common_judgement)
    fleiss_table[i] = [model_judgements.count("Candidate 1"), model_judgements.count("Candidate 2"), model_judgements.count(TIE_WORD)]


  print("Total examples: ", tot_examples)
  print("Agreement between models: ", sum(instance_wise_agreements) / tot_examples)
  print("Non-tie agreement between models: ", sum(non_tie_instance_wise_agreements) / len(non_tie_instance_wise_agreements))
  print("Candidate 1 win rate: ", sum([1 for x in majority_judgements if x == "Candidate 1"]) / len(majority_judgements))
  print("Candidate 2 win rate: ", sum([1 for x in majority_judgements if x == "Candidate 2"]) / len(majority_judgements))
  print("Tie rate: ", sum([1 for x in majority_judgements if x == TIE_WORD]) / len(majority_judgements))
...
def main(unused_argv) -> None:
  ...
  if "claude-3.5-sonnet_vs_meta-llama_Meta-Llama-3.1-405B-Instruct-Turbo" in _EVAL_OUTPUTS.value:
    eval_models = ["gpt-4", "gemini-1.5-pro", "google_gemma-2-27b-it", "jamba-1.5-large", "Qwen_Qwen2-72B-Instruct"]
  elif "google_gemma-2-27b-it_vs_jamba-1.5-large" in _EVAL_OUTPUTS.value:
    eval_models = ["claude-3.5-sonnet", "meta-llama_Meta-Llama-3.1-405B-Instruct-Turbo", "gpt-4", "gemini-1.5-pro", "Qwen_Qwen2-72B-Instruct"]
  elif "gpt-4_vs_gemini-1.5-flash-exp-0827" in _EVAL_OUTPUTS.value:
    eval_models = ["claude-3.5-sonnet", "meta-llama_Meta-Llama-3.1-405B-Instruct-Turbo", "google_gemma-2-27b-it", "jamba-1.5-large", "Qwen_Qwen2-72B-Instruct"]
  ...
  wo_ctx_instance_wise_agreements, wo_ctx_non_tie_instance_wise_agreements = get_agreement(wo_ctx_eval_judgements, eval_models, selected_idxs)
  ...
  gen_wo_ctx_eval_w_ctx_instance_wise_agreements, gen_wo_ctx_eval_w_ctx_non_tie_instance_wise_agreements = get_agreement(gen_wo_ctx_eval_w_ctx_eval_judgements, eval_models, selected_idxs)
  ...
  w_ctx_instance_wise_agreements, w_ctx_non_tie_instance_wise_agreements = get_agreement(w_ctx_eval_judgements, eval_models, selected_idxs)
  ...
  ttest_res = ttest_rel(wo_ctx_instance_wise_agreements, gen_wo_ctx_eval_w_ctx_instance_wise_agreements)
  print("T-test between gen_wo_ctx_eval_wo_ctx and gen_wo_ctx_eval_w_ctx (with ties):", ttest_res)
  if ttest_res.pvalue > 0.05:
    sig[0] = False

  ttest_res = ttest_ind(wo_ctx_non_tie_instance_wise_agreements, gen_wo_ctx_eval_w_ctx_non_tie_instance_wise_agreements)
  print("T-test between gen_wo_ctx_eval_wo_ctx and gen_wo_ctx_eval_w_ctx (without ties):", ttest_res)
  if ttest_res.pvalue > 0.05:
    sig[1] = False
  ...
  ttest_res = ttest_ind(wo_ctx_non_tie_instance_wise_agreements, w_ctx_non_tie_instance_wise_agreements)
  print("T-test between gen_wo_ctx_eval_wo_ctx and gen_w_ctx_eval_w_ctx (without ties):", ttest_res)
  if ttest_res.pvalue > 0.05:
    sig[3] = False

  ttest_res = ttest_rel(gen_wo_ctx_eval_w_ctx_instance_wise_agreements, w_ctx_instance_wise_agreements)
  print("T-test between gen_wo_ctx_eval_w_ctx and gen_w_ctx_eval_w_ctx (with ties):", ttest_res)
  if ttest_res.pvalue > 0.05:
    sig[4] = False

  ttest_res = ttest_ind(gen_wo_ctx_eval_w_ctx_non_tie_instance_wise_agreements, w_ctx_non_tie_instance_wise_agreements)
  print("T-test between gen_wo_ctx_eval_w_ctx and gen_w_ctx_eval_w_ctx (without ties):", ttest_res)
  if ttest_res.pvalue > 0.05:
    sig[5] = False

  if sum(sig)==6:
    print(u"\u2713\u2713\u2713\u2713\u2713\u2713 All differences are significant!!! \u2713\u2713\u2713\u2713\u2713\u2713")
  else:
